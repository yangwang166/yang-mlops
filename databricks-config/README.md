...
principal corresponding to a particular environment has permissions to read the input Delta table and modify the output Delta table in that environment's workspace. If the Delta table is in the Unity Catalog, these permissions are
...
resource "databricks_job" "batch_inference_job" {
  name = "yang-mlops-project batch inference job"
...
The example above defines a Databricks job with name `yang-mlops-project batch inference job`
